...
# local <- Sys.info()[["nodename"]] == "C02FC5BLMD6M"
local <- FALSE
knitr::opts_knit$set(root.dir = ifelse(local, "~/NUP98/", file.path(clinical,"analysis","2020.03.25_NUP98-Rearranged_AML_Collaboration")))
...
```{r}
nup.ineligable <- openxlsx::read.xlsx("COG/nup98x_notincohort.xlsx", sheet = 1) %>% 
  filter(!Not_in == "") %>% 
  arrange(Reg_NO)
...
```{r}
exon_juncs <- read.csv("TARGET_AML_NUP98_fusion_exons_3.20.21.csv") %>% 
...
```{r}
manifest <- read.csv("TARGET_AML_RNAseq_Cohort_for_Manuscript.csv")
